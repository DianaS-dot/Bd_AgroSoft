...
@Injectable()
export class CultivoHistorialPostgresRepository implements CultivoHistorialRepository {
  constructor(
    @InjectRepository(CultivoHistorialOrmEntity)
    private readonly repository: Repository<CultivoHistorialOrmEntity>,
  ) {}

  async crear(historial: CultivoHistorial): Promise<CultivoHistorial> {
  ...
  async obtenerPorId(id: number): Promise<CultivoHistorial | null> {
  ...
  async actualizar(historial: CultivoHistorial): Promise<CultivoHistorial> {
